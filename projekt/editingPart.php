<?php
	...
	if (isset($_POST['edit']))
	{
		...
		if (empty($naziv))
		{
			$greska = array('box'=>'style="border: 1pt solid #FF0000"', 'greska'=>'Unesite naziv dijela!');
			$smarty->assign("name", $greska);
			$greske++;  
		}
		else
		{
			$vrijednost = array('vrijednost'=>$naziv);
			$smarty->assign("name",$vrijednost);
		}
		
		if (empty($opis))
		{
			$greska = array('box'=>'style="border: 1pt solid #FF0000"', 'greska'=>'Unesite opis!');
			$smarty->assign("desc",$greska);
			$greske++;  
		}
		else
		{
			$vrijednost = array('vrijednost'=>$opis);
			$smarty->assign("desc",$vrijednost);
		}
		
		if (empty($cijena))
		{
			$greska = array('box'=>'style="border: 1pt solid #FF0000"', 'greska'=>'Unesite cijenu!');
			$smarty->assign("price",$greska);
			$greske++; 
		}
		else
		{
			$vrijednost = array('vrijednost'=>$cijena);
			$smarty->assign("price",$vrijednost);
		}
		
		$conn = new DBConfig();
		$result = mysql_query("SELECT iddio FROM dijelovi WHERE naziv = '$naziv' and opis = '$opis'");
		...
		if($_FILES["slika"]["error"]  == UPLOAD_ERR_OK)
		{
			...
			{
				$dir = "slike/dijelovi/$naziv/$opis/";
				if (!is_dir($dir))
				{
					mkdir("slike/dijelovi/$naziv/$opis/", 0777);
				}
				...
				$dir = "slike/dijelovi/$naziv/$opis/thumb/";
				if (!is_dir($dir))
				{
					mkdir("slike/dijelovi/$naziv/$opis/thumb/", 0777);
				}
				$remote_file = "slike/dijelovi/$naziv/$opis/thumb/".$_FILES["slika"]["name"];
				...
				$thumbPath = "slike/dijelovi/$naziv/$opis/thumb/".$_FILES['slika']['name'];
		...
		{			
			...
			if(!empty($path))
			{
				...
				mysql_query("update dijelovi set naziv='$naziv', opis='$opis', cijena='$cijena', akcija='$akcija', path_slika='$path', path_thumb='$thumbPath' where iddio='$partID'") or die(mysql_error());
			}
			else
			{
				mysql_query("update dijelovi set naziv='$naziv', opis='$opis', cijena='$cijena', akcija='$akcija' where iddio='$partID'") or die(mysql_error());
			}
			$conn::zatvori();
